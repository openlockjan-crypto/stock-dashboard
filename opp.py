...
with tab3:
    ...
    with col_tools2:
        st.caption("👈 勾選「移除」後按刪除。請直接在表格輸入數值。")

    # [關鍵修改] 這裡不使用 callback，改用最原始的 key 綁定
    # 並且不將返回值賦值回去，減少循環依賴
    ...
    # 手動同步：只有當我們準備好要用的時候，才把編輯器的內容拿來用
    # 但為了讓編輯器記住狀態，我們需要把 edited_df 存起來，但不要馬上觸發 rerun
    if not edited_df.equals(st.session_state.my_portfolio_data):
    ...
    if st.button("🔄 刷新即時報價", type="primary"):
        with st.spinner("正在連線 Alpaca 抓取最新股價..."):
            # 使用最新的 edited_df 進行計算
            df, total_val, errs = get_portfolio_data(api_key, secret_key, edited_df)
            st.session_state.portfolio_df = df
            st.session_state.total_val = total_val
            if errs: st.toast(f"部分代號抓取失敗: {len(errs)}", icon="⚠️")

    # 結果顯示 (省略部分樣式設定以節省長度，功能完全保留)
